...
DEFAULT_DATA = "data/mixed_policy/tau2_airline_mixed_dialogue_tool_policy_v1_2048_heldout.jsonl"
DEFAULT_OUTPUT_DIR = "outputs/behavior_sft_mixed_dialogue_tool_policy_v1"
DEFAULT_REPORT = "reports/behavior_sft_mixed_dialogue_tool_policy_v1.md"
...
def reference_body(message: dict[str, Any]) -> str:
    segment, _spans = render_message(
        message,
        target=False,
        assistant_content_loss=False,
...
def build_probes(rows: list[dict[str, Any]], max_probes: int, include_text: bool, include_tools: bool) -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        reference_calls = [normalize_call(call) for call in target.get("tool_calls") or []]
        action = "tool_call" if reference_calls else "assistant_text"
        if action == "assistant_text" and not include_text:
            continue
        if action == "tool_call" and not include_tools:
            continue
        probes.append(
            {
                ...
                "reference_body": reference_body(target),
                "reference_calls": reference_calls,
                "reference_tool_names": [call["name"] for call in reference_calls],
            }
        )
...
def generate_one(
    ...
    pred_action = "tool_call" if calls else ("assistant_text" if body else "empty")
    return {
...
def score_generation(probe: dict[str, Any], generation: dict[str, Any]) -> dict[str, Any]:
    ref_action = probe["target_action"]
    pred_action = generation["predicted_action"]
    ...
    return {
        "action_type_match": ref_action == pred_action or (ref_action == "assistant_text" and pred_action == "assistant_text"),
        "text_probe_no_tool": ref_action == "assistant_text" and not pred_calls,
        "text_probe_nonempty": ref_action == "assistant_text" and bool(generation["generated_body"]),
        "tool_probe_has_tool": ref_action == "tool_call" and bool(pred_calls),
        "tool_name_match": tool_name_match if ref_action == "tool_call" else False,
        "arg_exact_match": arg_exact_match if ref_action == "tool_call" else False,
    }
...
def evaluate_model(
    ...
    for idx, probe in enumerate(probes, start=1):
        ...
        print(
            ...
            f"ref_action={probe['target_action']} pred_action={generation['predicted_action']} "
            f"tool_match={score['tool_name_match']} probe={probe['probe_id']}"
        )
...
def summarize(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    tool_call_counts = [len(row.get("generated_calls") or []) for row in tool_rows]
    return {
        "probes": len(rows),
        "text_probes": len(text_rows),
        "tool_probes": len(tool_rows),
        "action_type_accuracy": ratio(sum(1 for row in rows if row["action_type_match"]), len(rows)),
        "text_no_tool_rate": ratio(sum(1 for row in text_rows if row["text_probe_no_tool"]), len(text_rows)),
        "text_nonempty_rate": ratio(sum(1 for row in text_rows if row["text_probe_nonempty"]), len(text_rows)),
        "tool_call_rate": ratio(sum(1 for row in tool_rows if row["tool_probe_has_tool"]), len(tool_rows)),
        "protocol_wrapper_rate": ratio(sum(1 for row in tool_rows if row["has_protocol_wrapper"]), len(tool_rows)),
        "single_call_rate": ratio(sum(1 for count in tool_call_counts if count == 1), len(tool_rows)),
        "multi_call_rate": ratio(sum(1 for count in tool_call_counts if count > 1), len(tool_rows)),
        "tool_name_accuracy": ratio(sum(1 for row in tool_rows if row["tool_name_match"]), len(tool_rows)),
        "arg_exact_accuracy": ratio(sum(1 for row in tool_rows if row["arg_exact_match"]), len(tool_rows)),
        "json_parse_failures": sum(1 for row in rows if row["json_parse_failed"]),
        "predicted_actions": dict(sorted(Counter(row["predicted_action"] for row in rows).items())),
        "reference_tools": dict(sorted(Counter((row["reference_tool_names"] or [""])[0] for row in tool_rows).items())),
    }
...
def write_report(path: Path, payload: dict[str, Any]) -> None:
    ...
    for result in payload["results"]:
        ...
        lines.append(
            f"| `{result['name']}` | {summary['action_type_accuracy']:.3f} | "
            f"{summary['text_no_tool_rate']:.3f} | {summary['text_nonempty_rate']:.3f} | "
            f"{summary['tool_call_rate']:.3f} | {summary['protocol_wrapper_rate']:.3f} | "
            f"{summary['single_call_rate']:.3f} | {summary['multi_call_rate']:.3f} | "
    ...
    for result in payload["results"]:
        ...
        for row in result["rows"]:
            ref_tool = ",".join(row["reference_tool_names"]) or "-"
            pred_tool = ",".join(call.get("name", "") for call in row["generated_calls"]) or "-"
            lines.append(
                f"| `{row['probe_id']}` | {row['task_id']} | `{row['target_action']}` | "
                f"`{row['predicted_action']}` | `{ref_tool}` | `{pred_tool}` | "
                f"{'Y' if row['action_type_match'] else 'N'} | `{compact_generated(row['generated_body'])}` |"
            )
        lines.append("")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate mixed text/tool policy behavior.")
    ...
    parser.add_argument("--include-tools", action=argparse.BooleanOptionalAction, default=True)
...
def main() -> int:
    ...
    probes = build_probes(rows, max_probes=args.max_probes, include_text=args.include_text, include_tools=args.include_tools)
    if not probes:
        raise RuntimeError("No mixed policy probes found.")
    ...
    for name, adapter_path in model_specs:
        ...
        write_json(output_dir / f"{name}_mixed_behavior.json", result)
