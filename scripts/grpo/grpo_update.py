...
def load_policy(base_model: str, adapter_in: str | None, targets: list[str],
    ...
    model = AutoModelForCausalLM.from_pretrained(
        ...
        # sdpa avoids materializing the full LxL attention matrix — eager OOM'd at 20K-token
        # teacher trajectories (28 heads x 19218^2 fp32 = one 38.5GB alloc; fine <=4K, fatal >=16K)
...
def rollout_loss(model, device, ids: list[int], mask: list[int], advantage: float, lata: bool,
                 td_alpha: float = 0.0) -> torch.Tensor | None:
    # td_alpha > 1 enables Turn-Discounted advantage: weight each assistant token by
    # alpha^((N-1)-rank) (rank = its 0-indexed position among the N assistant tokens, in order),
    # mean-normalized to 1 -> EARLY tokens get more advantage, protecting early-turn reasoning
    # from the "quantity over quality" degeneration. A per-token REWEIGHT (orthogonal to LATA's
    # 1/sqrt(L) NORM), applied on the masked positions only.
    n_asst = sum(mask)
    if n_asst == 0:
        return None
    input_ids = torch.tensor([ids], dtype=torch.long, device=device)
    # LONG-SEQUENCE MEMORY DISCIPLINE (20K-token teacher trajectories OOM'd three ways at 32GB):
    # (1) NO attention_mask: batch=1 has no padding, and an explicit all-ones mask pushes SDPA
    #     off the flash/is_causal path onto the math backend -> full LxL fp32 attention matrix
    #     (28 heads x 19218^2 = 38.5GB).
    # (2) NEVER materialize full-sequence logits: [19K, 152K vocab] is 5.9GB bf16 + 11GB fp32
    #     inside cross_entropy + 11GB saved by its autograd. Instead run the BACKBONE (hidden
    #     states are only [T, 3584] = 0.14GB), then lm_head+CE per 4096-token CHUNK under
    #     torch.utils.checkpoint -> transients are chunk-sized and recomputed in backward.
    # LoRA layers live inside the backbone modules, so calling it directly keeps them active.
    core = model.get_base_model() if hasattr(model, "get_base_model") else model
    hidden = core.model(input_ids=input_ids).last_hidden_state[:, :-1, :]  # [1, T-1, H]
    flat_h = hidden.reshape(-1, hidden.size(-1))
    tgt = input_ids[:, 1:].reshape(-1)
    def _chunk_ce(h, t):
        return F.cross_entropy(core.lm_head(h), t, reduction="none")
    parts = []
    for s in range(0, flat_h.size(0), 4096):
        parts.append(-torch.utils.checkpoint.checkpoint(
            _chunk_ce, flat_h[s:s + 4096], tgt[s:s + 4096], use_reentrant=False))
    logp = torch.cat(parts)  # [T-1]
    mshift = torch.tensor(mask[1:], dtype=torch.float32, device=device)
    if td_alpha and td_alpha > 1.0:
        pos = torch.nonzero(mshift > 0, as_tuple=False).flatten()  # assistant-token positions, in order
        n = pos.numel()
        if n > 0:
            ranks = torch.arange(n, dtype=torch.float32, device=device)
            wv = td_alpha ** ((n - 1) - ranks)   # early tokens (small rank) -> high weight
            wv = wv / wv.mean()                  # mean-normalize -> overall scale unchanged
            mshift = mshift.clone()
            mshift[pos] = wv
...
def main() -> int:
    ...
    ap.add_argument("--td-alpha", type=float, default=0.0,
                    help="Turn-Discounted advantage: >1 (e.g. 1.05) weights early assistant tokens "
                         "alpha^((N-1)-rank), mean-normalized. 0=off. Only the batch-size=1 path.")
    ...
    if args.td_alpha and args.td_alpha > 1.0 and args.batch_size > 1:
        raise SystemExit("[grpo] --td-alpha is only wired for the batch-size=1 path; use --batch-size 1.")
    ...
    for ep in range(args.epochs):
        ...
        if args.batch_size > 1:
            ...
            for k, (ids, mask, adv) in enumerate(prepared):
                loss = rollout_loss(model, device, ids, mask, adv, args.lata, args.td_alpha)
                if loss is None or not torch.isfinite(loss):
                    continue
    ...
    metrics = {
        ...
        "reward_mode": args.reward_mode, "lata": args.lata, "td_alpha": args.td_alpha, "lr": args.lr,
