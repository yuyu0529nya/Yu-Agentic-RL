...
# show progress in the TERMINAL and also keep the full log (foreground = watch live)
...
# SAME-DAY 2x2 ablation (one usersim instance -> clean 4-way; cross-day reuse abandoned
# after base drifted 0.190->0.240 between 06-29 and 06-30):
#   vanilla  = binary,   no LATA      prmlata  = prm_lite, +LATA
#   prmonly  = prm_lite, no LATA      lataonly = binary,   +LATA
...
$PYBIN scripts/grpo/tau2_eval_analyze.py "$OUT/base_eval.jsonl" \
  vanilla="$OUT/vanilla_eval_final.jsonl" prmlata="$OUT/prmlata_eval_final.jsonl" \
  prmonly="$OUT/prmonly_eval_final.jsonl" lataonly="$OUT/lataonly_eval_final.jsonl" 2>&1 || echo "analyze failed"
echo "====== per-iter curves ======"
for nm in vanilla prmlata prmonly lataonly; do for ((i=1;i<=ITERS;i++)); do f="$OUT/${nm}_eval_it${i}.jsonl"; [ -f "$f" ] && echo "$nm it$i: $(passrate "$f")"; done; done
