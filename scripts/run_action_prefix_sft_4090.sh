...
# First rented-GPU run for Action-Prefix SFT.
# Defaults target a single RTX 4090 24G with QLoRA.
...
STEM="tau2_airline_action_prefix_v2_${MAX_SAMPLE_TOKENS}"
RUN_NAME="sft_action_prefix_v2_qwen25_7b_qlora_${MAX_SAMPLE_TOKENS}"

python scripts/build_action_prefix_dataset_v2.py \
  --tokenizer-model "${BASE_MODEL}" \
  --max-sample-tokens "${MAX_SAMPLE_TOKENS}" \
  --output-stem "${STEM}" \
  --report "reports/action_prefix_dataset_v2_${MAX_SAMPLE_TOKENS}.md" \
  --manifest "data/action_prefix/tau2_airline_action_prefix_manifest_v2_${MAX_SAMPLE_TOKENS}.json"
