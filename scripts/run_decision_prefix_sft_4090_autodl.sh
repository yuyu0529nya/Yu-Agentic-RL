...
# AutoDL wrapper for Decision-Prefix SFT.
...
ARTIFACT_TAR="${ARTIFACT_TAR:-/root/autodl-tmp/yuyu_decision_prefix_artifacts_$(date '+%Y%m%d_%H%M%S').tar.gz}"
...
export MODEL_REPO MODEL_DIR MAX_SAMPLE_TOKENS STEPS LR LORA_R LORA_ALPHA MAX_TARGET_CONTENT_CHARS
...
package_artifacts() {
  ...
  tar -czf "${ARTIFACT_TAR}" \
    run_decision_prefix_4090.log \
    "reports/decision_prefix_dataset_v1_${MAX_SAMPLE_TOKENS}.md" \
    "reports/${RUN_NAME}.md" \
    "reports/behavior_${RUN_NAME}.md" \
    "data/decision_prefix/tau2_airline_decision_prefix_manifest_v1_${MAX_SAMPLE_TOKENS}.json" \
...
echo "[$(date '+%F %T')] AutoDL decision-prefix SFT run started"
pwd
nvidia-smi || true
python -V
...
MAX_TARGET_CONTENT_CHARS="${MAX_TARGET_CONTENT_CHARS}" \
bash scripts/run_decision_prefix_sft_4090.sh 2>&1 | tee run_decision_prefix_4090.log
