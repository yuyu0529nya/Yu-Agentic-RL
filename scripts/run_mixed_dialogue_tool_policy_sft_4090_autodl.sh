...
# AutoDL wrapper for Phase2H mixed dialogue/tool-policy SFT.
...
ARTIFACT_TAR="${ARTIFACT_TAR:-/root/autodl-tmp/yuyu_mixed_dialogue_tool_policy_artifacts_$(date '+%Y%m%d_%H%M%S').tar.gz}"
export MODEL_REPO MODEL_DIR MAX_SAMPLE_TOKENS STEPS LR LORA_R LORA_ALPHA MAX_NEW_TOKENS EVAL_PROBES
...
package_artifacts() {
  ...
  tar -czf "${ARTIFACT_TAR}" \
    run_mixed_dialogue_tool_policy_4090.log \
    "reports/mixed_dialogue_tool_policy_dataset_v1_${MAX_SAMPLE_TOKENS}.md" \
    "reports/${RUN_NAME}.md" \
    "reports/behavior_${RUN_NAME}.md" \
    "data/mixed_policy/tau2_airline_mixed_dialogue_tool_policy_manifest_v1_${MAX_SAMPLE_TOKENS}.json" \
    "data/mixed_policy/tau2_airline_mixed_dialogue_tool_policy_v1_${MAX_SAMPLE_TOKENS}_train.jsonl" \
    "data/mixed_policy/tau2_airline_mixed_dialogue_tool_policy_v1_${MAX_SAMPLE_TOKENS}_valid.jsonl" \
    "data/mixed_policy/tau2_airline_mixed_dialogue_tool_policy_v1_${MAX_SAMPLE_TOKENS}_heldout.jsonl" \
...
echo "[$(date '+%F %T')] AutoDL mixed dialogue/tool-policy run started"
pwd
nvidia-smi || true
python -V
...
bash scripts/run_mixed_dialogue_tool_policy_sft_4090.sh 2>&1 | tee run_mixed_dialogue_tool_policy_4090.log
