...
# AutoDL wrapper for slot-grounded Action-Prefix SFT.
...
ARTIFACT_TAR="${ARTIFACT_TAR:-/root/autodl-tmp/yuyu_slot_grounded_action_prefix_artifacts_$(date '+%Y%m%d_%H%M%S').tar.gz}"
...
package_artifacts() {
  ...
  tar -czf "${ARTIFACT_TAR}" \
    run_slot_grounded_action_prefix_4090.log \
    "reports/action_prefix_slot_grounded_dataset_v3_${MAX_SAMPLE_TOKENS}.md" \
    "reports/${RUN_NAME}.md" \
    "reports/behavior_${RUN_NAME}.md" \
    "data/action_prefix/tau2_airline_action_prefix_slot_grounded_manifest_v3_${MAX_SAMPLE_TOKENS}.json" \
...
echo "[$(date '+%F %T')] AutoDL slot-grounded action-prefix run started"
pwd
nvidia-smi || true
python -V
...
bash scripts/run_slot_grounded_action_prefix_sft_4090.sh 2>&1 | tee run_slot_grounded_action_prefix_4090.log
