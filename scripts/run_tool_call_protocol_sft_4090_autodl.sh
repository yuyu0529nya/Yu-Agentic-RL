...
# AutoDL wrapper for Tool-Call Protocol SFT.
...
ARTIFACT_TAR="${ARTIFACT_TAR:-/root/autodl-tmp/yuyu_tool_call_protocol_artifacts_$(date '+%Y%m%d_%H%M%S').tar.gz}"
...
package_artifacts() {
  ...
  tar -czf "${ARTIFACT_TAR}" \
    run_tool_call_protocol_4090.log \
    "reports/tool_call_protocol_dataset_v1_${MAX_SAMPLE_TOKENS}.md" \
    "reports/${RUN_NAME}.md" \
...
echo "[$(date '+%F %T')] AutoDL protocol SFT run started"
pwd
nvidia-smi || true
python -V
...
bash scripts/run_tool_call_protocol_sft_4090.sh 2>&1 | tee run_tool_call_protocol_4090.log
